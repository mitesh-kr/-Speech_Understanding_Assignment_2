...
for language, stats in mfcc_stats.items():
    plt.plot(stats["mean"], label=f"{language} Mean MFCC")
plt.title("Comparison of Mean MFCCs Across Languages")
plt.xlabel("MFCC Coefficients")
plt.ylabel("Mean Value")
plt.legend()
plt.grid()
plt.show()
plt.savefig(os.path.join(SAVE_DIR, "mean_mfcc_comparison.png"), dpi=300)
plt.close()

plt.figure(figsize=(12, 6))
for language, stats in mfcc_stats.items():
    plt.plot(stats["variance"], label=f"{language} Variance MFCC")
plt.title("Comparison of Variance of MFCCs Across Languages")
plt.xlabel("MFCC Coefficients")
plt.ylabel("Variance Value")
plt.legend()
plt.grid()
plt.show()
plt.savefig(os.path.join(SAVE_DIR, "variance_mfcc_comparison.png"), dpi=300)
plt.close()

print(f"✅ MFCC visualization and statistical comparison saved in {SAVE_DIR}")